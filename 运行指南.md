# 🚀 药物筛选系统 - 完整运行指南

## 📑 目录
1. [环境安装](#环境安装)
2. [快速开始](#快速开始)
3. [完整训练流程](#完整训练流程)
4. [Web界面使用](#web界面使用)
5. [常见问题](#常见问题)

---

## 🔧 环境安装

### 方式一: 使用Conda（推荐）

#### 步骤1: 安装Anaconda
下载并安装: https://www.anaconda.com/download

#### 步骤2: 创建虚拟环境
```bash
# 创建Python 3.9环境
conda create -n drug_screen python=3.9 -y

# 激活环境
conda activate drug_screen
```

#### 步骤3: 安装RDKit（必须用conda）
```bash
conda install -c conda-forge rdkit -y
```

#### 步骤4: 安装PyTorch
```bash
# CPU版本
conda install pytorch torchvision torchaudio cpuonly -c pytorch -y

# GPU版本（如果有NVIDIA显卡）
conda install torch torchvision torchaudio pytorch-cuda=11.8 -c pytorch -c nvidia -y
```

#### 步骤5: 安装其他依赖
```bash
# 进入项目目录
cd d:\OneDrive\桌面\cursor\drug

# 安装依赖
pip install deepchem pandas scikit-learn matplotlib seaborn streamlit tqdm pyyaml
```

### 方式二: 使用requirements.txt

```bash
# 激活环境后
pip install -r requirements.txt
```

**注意**: RDKit必须通过conda安装，pip版本可能有问题。

### 验证安装

创建测试脚本 `test_env.py`:
```python
import sys
print("Python版本:", sys.version)

try:
    import torch
    print("✓ PyTorch:", torch.__version__)
    print("  CUDA可用:", torch.cuda.is_available())
except:
    print("✗ PyTorch未安装")

try:
    from rdkit import Chem
    print("✓ RDKit已安装")
    mol = Chem.MolFromSmiles('CCO')
    print("  SMILES解析测试通过")
except:
    print("✗ RDKit未安装或有问题")

try:
    import deepchem as dc
    print("✓ DeepChem:", dc.__version__)
except:
    print("✗ DeepChem未安装")

try:
    import pandas as pd
    import numpy as np
    import streamlit as st
    print("✓ 其他依赖已安装")
except:
    print("✗ 部分依赖缺失")
```

运行测试:
```bash
python test_env.py
```

---

## 🎯 快速开始

### 示例1: 测试数据加载

创建 `demo_01_data.py`:
```python
"""演示数据加载"""
import sys
sys.path.append('.')

from data.data_loader import DrugDataLoader

# 创建加载器
loader = DrugDataLoader(data_dir='./data/raw')

# 加载MoleculeNet的BBBP数据集
print("正在下载BBBP数据集...")
train, valid, test, tasks = loader.load_moleculenet_dataset(
    dataset_name='BBBP',
    featurizer='ECFP',
    split='scaffold'
)

print(f"\n数据集已加载:")
print(f"  训练集: {len(train)} 样本")
print(f"  验证集: {len(valid)} 样本")
print(f"  测试集: {len(test)} 样本")
print(f"  任务: {tasks}")

# 查看第一个样本
print(f"\n第一个训练样本:")
print(f"  特征形状: {train.X[0].shape}")
print(f"  标签: {train.y[0]}")
```

运行:
```bash
python demo_01_data.py
```

### 示例2: 测试特征提取

创建 `demo_02_features.py`:
```python
"""演示特征提取"""
from features.molecular_features import MolecularFeaturizer

# 创建特征提取器
featurizer = MolecularFeaturizer(fingerprint_size=2048)

# 示例分子
smiles_list = [
    'CC(=O)OC1=CC=CC=C1C(=O)O',  # 阿司匹林
    'CN1C=NC2=C1C(=O)N(C(=O)N2C)C',  # 咖啡因
    'CC(C)CC1=CC=C(C=C1)C(C)C(=O)O'  # 布洛芬
]

print("提取Morgan指纹...")
for i, smiles in enumerate(smiles_list):
    # Morgan指纹
    fp = featurizer.get_morgan_fingerprint(smiles)
    print(f"\n分子 {i+1}: {smiles}")
    print(f"  指纹维度: {fp.shape}")
    print(f"  非零位数: {fp.sum()}")
    
    # 分子描述符
    desc = featurizer.get_molecular_descriptors(smiles)
    print(f"  分子量: {desc['MolecularWeight']:.2f} Da")
    print(f"  LogP: {desc['LogP']:.2f}")
    print(f"  TPSA: {desc['TPSA']:.2f}")

# 批量特征化
print("\n批量特征化...")
features = featurizer.batch_featurize(smiles_list, method='morgan')
print(f"特征矩阵形状: {features.shape}")
```

运行:
```bash
python demo_02_features.py
```

---

## 🏋️ 完整训练流程

### 训练脚本: `train_model.py`

```python
"""
完整的模型训练脚本
用于BBBP数据集（血脑屏障穿透性预测）
"""
import torch
import numpy as np
from sklearn.model_selection import train_test_split
import sys
sys.path.append('.')

from data.data_loader import DrugDataLoader
from features.molecular_features import MolecularFeaturizer, FeatureScaler
from models.drug_models import DrugPredictorMLP
from training.trainer import DrugModelTrainer, create_data_loaders
from evaluation.metrics import ModelEvaluator, ResultVisualizer

def main():
    print("="*60)
    print("药物筛选模型训练 - BBBP数据集")
    print("="*60)
    
    # ==================== 1. 数据加载 ====================
    print("\n[1/6] 加载数据...")
    loader = DrugDataLoader(data_dir='./data/raw')
    
    train_data, valid_data, test_data, tasks = loader.load_moleculenet_dataset(
        dataset_name='BBBP',
        featurizer='ECFP',
        split='scaffold'
    )
    
    print(f"✓ 数据加载完成")
    
    # ==================== 2. 特征提取 ====================
    print("\n[2/6] 提取分子特征...")
    featurizer = MolecularFeaturizer(fingerprint_size=2048)
    
    # 从DeepChem数据集中提取SMILES和标签
    # 注意: DeepChem已经特征化了，我们需要重新提取SMILES
    # 这里简化处理，直接使用已有的特征
    X_train = train_data.X
    y_train = train_data.y.flatten()
    
    X_valid = valid_data.X
    y_valid = valid_data.y.flatten()
    
    X_test = test_data.X
    y_test = test_data.y.flatten()
    
    print(f"✓ 特征提取完成")
    print(f"  训练集: {X_train.shape}")
    print(f"  验证集: {X_valid.shape}")
    print(f"  测试集: {X_test.shape}")
    
    # ==================== 3. 创建数据加载器 ====================
    print("\n[3/6] 创建数据加载器...")
    train_loader, val_loader = create_data_loaders(
        X_train, y_train,
        X_valid, y_valid,
        batch_size=64
    )
    print(f"✓ 数据加载器创建完成 (batch_size=64)")
    
    # ==================== 4. 创建模型 ====================
    print("\n[4/6] 创建模型...")
    model = DrugPredictorMLP(
        input_dim=2048,
        hidden_dims=[1024, 512, 256],
        output_dim=1,
        dropout=0.3,
        task_type='binary'
    )
    
    print(f"✓ 模型创建完成")
    print(f"  参数量: {sum(p.numel() for p in model.parameters()):,}")
    
    # ==================== 5. 训练模型 ====================
    print("\n[5/6] 开始训练...")
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"  使用设备: {device}")
    
    trainer = DrugModelTrainer(
        model=model,
        device=device,
        learning_rate=0.0001,
        task_type='binary'
    )
    
    trainer.fit(
        train_loader=train_loader,
        val_loader=val_loader,
        epochs=100,
        early_stopping_patience=15,
        save_best_model=True,
        model_save_path='./saved_models/bbbp_model.pth'
    )
    
    print(f"\n✓ 训练完成！")
    
    # ==================== 6. 评估模型 ====================
    print("\n[6/6] 评估模型...")
    
    # 加载最佳模型
    model.load_state_dict(torch.load('./saved_models/bbbp_model.pth', 
                                     map_location=device))
    
    # 在测试集上预测
    y_pred = trainer.predict(X_test)
    y_pred_labels = (y_pred > 0.5).astype(int).flatten()
    
    # 计算指标
    evaluator = ModelEvaluator()
    metrics = evaluator.evaluate_classification(
        y_test.astype(int),
        y_pred_labels,
        y_prob=y_pred.flatten()
    )
    
    evaluator.print_metrics(metrics, "BBBP测试集评估结果")
    
    # 可视化
    print("\n生成可视化图表...")
    visualizer = ResultVisualizer(save_dir='./evaluation/figures')
    
    # 训练历史
    visualizer.plot_training_history(
        trainer.history,
        save_name='bbbp_training_history.png'
    )
    
    # ROC曲线
    visualizer.plot_roc_curve(
        y_test.astype(int),
        y_pred.flatten(),
        title='BBBP模型ROC曲线',
        save_name='bbbp_roc_curve.png'
    )
    
    # 混淆矩阵
    visualizer.plot_confusion_matrix(
        y_test.astype(int),
        y_pred_labels,
        labels=['不穿透', '穿透'],
        title='BBBP预测混淆矩阵',
        save_name='bbbp_confusion_matrix.png'
    )
    
    print("\n" + "="*60)
    print("训练流程完成！")
    print("="*60)
    print(f"模型保存位置: ./saved_models/bbbp_model.pth")
    print(f"图表保存位置: ./evaluation/figures/")
    print("="*60)

if __name__ == "__main__":
    main()
```

### 运行训练

```bash
# 激活环境
conda activate drug_screen

# 运行训练脚本
python train_model.py
```

### 预期输出

```
============================================================
药物筛选模型训练 - BBBP数据集
============================================================

[1/6] 加载数据...
Loading BBBP dataset from MoleculeNet...
✓ 数据加载完成

[2/6] 提取分子特征...
✓ 特征提取完成
  训练集: (1631, 2048)
  验证集: (204, 2048)
  测试集: (204, 2048)

[3/6] 创建数据加载器...
✓ 数据加载器创建完成 (batch_size=64)

[4/6] 创建模型...
✓ 模型创建完成
  参数量: 2,099,457

[5/6] 开始训练...
  使用设备: cuda
开始训练... Device: cuda
Epoch 1/100 - Train Loss: 0.6234, Train Accuracy: 0.6543 - Val Loss: 0.5876, Val Accuracy: 0.7010
  ✓ 保存最佳模型到 ./saved_models/bbbp_model.pth
Epoch 2/100 - Train Loss: 0.5123, Train Accuracy: 0.7234 - Val Loss: 0.4987, Val Accuracy: 0.7450
...
早停触发！在第 42 轮停止训练

✓ 训练完成！

[6/6] 评估模型...
==================================================
BBBP测试集评估结果
==================================================
Accuracy       : 0.8578
Precision      : 0.8421
Recall         : 0.8750
F1             : 0.8582
AUC-ROC        : 0.9123
==================================================
```

---

## 🌐 Web界面使用

### 启动Web应用

```bash
# 确保在项目根目录
cd d:\OneDrive\桌面\cursor\drug

# 启动Streamlit
streamlit run web/app.py
```

### 浏览器访问
自动打开浏览器访问: http://localhost:8501

### 功能说明

#### 1. 单分子预测
- 输入SMILES字符串
- 查看分子结构
- 获取预测分数
- 查看分子性质
- Lipinski规则检查

#### 2. 批量筛选
- 上传CSV文件（必须包含SMILES列）
- 设置Top-K参数
- 应用Lipinski过滤
- 查看筛选结果
- 下载结果CSV
- 查看Top分子结构

#### 3. 数据集探索
- 浏览MoleculeNet数据集信息
- 了解各数据集用途

#### 4. 系统说明
- 查看使用指南
- 技术文档

---

## 🔍 完整使用示例

### 场景1: 预测单个药物分子

```python
"""预测阿司匹林的血脑屏障穿透性"""
import torch
from features.molecular_features import MolecularFeaturizer
from models.drug_models import DrugPredictorMLP
from inference.predictor import DrugPredictor

# 加载模型
model = DrugPredictorMLP(input_dim=2048, output_dim=1, task_type='binary')
model.load_state_dict(torch.load('./saved_models/bbbp_model.pth', map_location='cpu'))
model.eval()

# 创建预测器
featurizer = MolecularFeaturizer()
predictor = DrugPredictor(model, featurizer, device='cpu')

# 预测
aspirin = 'CC(=O)OC1=CC=CC=C1C(=O)O'
result = predictor.predict_with_properties(aspirin)

print(f"分子: 阿司匹林")
print(f"SMILES: {aspirin}")
print(f"预测分数: {result['prediction']:.4f}")
print(f"血脑屏障穿透: {'是' if result['prediction'] > 0.5 else '否'}")
print(f"\n分子性质:")
for key, value in result['properties'].items():
    print(f"  {key}: {value:.2f}")
```

### 场景2: 批量筛选化合物库

```python
"""从1000个化合物中筛选Top100候选"""
import pandas as pd
from inference.predictor import DrugScreener

# 假设有CSV文件
df = pd.read_csv('compound_library.csv')
smiles_list = df['smiles'].tolist()

# 创建筛选器
screener = DrugScreener(predictor)

# 筛选
results = screener.screen_library(
    smiles_list=smiles_list,
    top_k=100,
    ascending=False
)

# 应用Lipinski过滤
filtered = screener.filter_by_lipinski(results)

# 保存结果
screener.generate_report(filtered, save_path='top_candidates.csv')

print(f"筛选完成！")
print(f"Top候选数: {len(filtered)}")
```

### 场景3: 训练ESOL溶解度模型

```python
"""训练回归模型预测溶解度"""
# 修改train_model.py中的参数

# 数据加载
train_data, valid_data, test_data, tasks = loader.load_moleculenet_dataset(
    dataset_name='ESOL',  # 改为ESOL
    featurizer='ECFP',
    split='random'
)

# 模型配置
model = DrugPredictorMLP(
    input_dim=2048,
    hidden_dims=[512, 256, 128],
    output_dim=1,
    dropout=0.2,
    task_type='regression'  # 回归任务
)

# 训练器
trainer = DrugModelTrainer(
    model=model,
    device=device,
    learning_rate=0.001,
    task_type='regression'  # 回归任务
)
```

---

## ❓ 常见问题解决

### Q1: 导入模块错误
```
ModuleNotFoundError: No module named 'rdkit'
```
**解决**:
```bash
conda install -c conda-forge rdkit
```

### Q2: CUDA out of memory
```
RuntimeError: CUDA out of memory
```
**解决**:
```python
# 方案1: 减小batch_size
batch_size = 16  # 从64改为16

# 方案2: 使用CPU
device = 'cpu'

# 方案3: 清空缓存
torch.cuda.empty_cache()
```

### Q3: DeepChem下载数据集失败
```
Error downloading dataset
```
**解决**:
```python
# 方法1: 重试几次（网络问题）
# 方法2: 手动下载后放到指定目录
# 方法3: 使用自定义CSV数据
loader.load_csv_dataset('my_data.csv')
```

### Q4: Streamlit启动失败
```
streamlit: command not found
```
**解决**:
```bash
pip install streamlit
```

### Q5: 模型预测结果全是0或1
**可能原因**: 模型过拟合或未训练好  
**解决**:
- 检查训练loss是否下降
- 增加训练epoch数
- 调整学习率
- 检查数据是否有问题

### Q6: SMILES解析失败
```
Invalid SMILES string
```
**解决**:
```python
from rdkit import Chem

# 验证SMILES
mol = Chem.MolFromSmiles(smiles)
if mol is None:
    print("无效的SMILES")
else:
    # 标准化
    canonical_smiles = Chem.MolToSmiles(mol)
```

---

## 📦 项目打包和分享

### 导出环境

```bash
# 导出conda环境
conda env export > environment.yml

# 或导出pip依赖
pip freeze > requirements.txt
```

### 重建环境

```bash
# 从environment.yml
conda env create -f environment.yml

# 从requirements.txt
pip install -r requirements.txt
```

### 文件清单

提交项目时应包含:
```
drug_screening_system/
├── data/
│   └── data_loader.py
├── features/
│   └── molecular_features.py
├── models/
│   └── drug_models.py
├── training/
│   └── trainer.py
├── evaluation/
│   ├── metrics.py
│   └── figures/  # 生成的图表
├── inference/
│   └── predictor.py
├── web/
│   └── app.py
├── saved_models/
│   └── bbbp_model.pth  # 训练好的模型
├── requirements.txt
├── 开发计划文档.md
├── 运行指南.md
├── train_model.py  # 训练脚本
└── README.md
```

---

## 🎓 学习资源

### 推荐教程
1. **RDKit入门**: https://www.rdkit.org/docs/GettingStartedInPython.html
2. **PyTorch基础**: https://pytorch.org/tutorials/beginner/basics/intro.html
3. **DeepChem教程**: https://deepchem.readthedocs.io/en/latest/get_started/tutorials.html

### 相关课程
- Coursera: AI for Medicine
- Fast.ai: Practical Deep Learning
- DataCamp: Machine Learning for Drug Discovery

### 论文阅读
- MoleculeNet (2018)
- Molecular Graph Convolutions (2017)
- ADMET prediction with Deep Learning (2019)

---

## 💬 技术支持

遇到问题？

1. **查看日志**: 检查终端输出的错误信息
2. **调试模式**: 使用`print()`打印中间结果
3. **测试模块**: 单独测试每个模块是否正常
4. **检查数据**: 确认数据格式正确
5. **版本兼容**: 确认依赖版本匹配

---

**祝你训练顺利！🎉**
